...
def image_prediction(image_path, result_filename=None, save_dir = "./image_prediction_results", confidence=0.5, model="./best.pt"):
    """
    ...
    # Run the model on the image with a lower confidence threshold to see weak detections
    ...
    print(f"\n--- Debug: Analyzing {os.path.basename(image_path)} ---")
    if len(detections) > 0:
        for i, (cls_id, conf_score) in enumerate(zip(detections.class_id, detections.confidence)):
            print(f"  Detection {i+1}: Class='{class_dict[cls_id]}' (ID: {cls_id}), Confidence={conf_score:.4f}")
    else:
        print("  No objects detected even with low confidence (0.1).")
...
if __name__ == '__main__':
    ...
    image_prediction("./dataset/black_faced_cuckoo_shrike/black_faced_cuckoo_shrike_def37a5c7ac8.jpg",result_filename='black_faced_cuckoo_shrike_detected_1.jpg')
